...
func (store *Datastore) processNode(ctx context.Context, root cid.Cid, getter *crdtNodeGetter, rootPrio uint64, delta *pb.Delta, node ipld.Node) ([]cid.Cid, error) {
	processor := store.NewNodeProcessor()

	ancestorResult, err := processor.checkAncestorReadiness(ctx, node)
	if err != nil {
		return nil, err
	}

	if !ancestorResult.AllReady {
		current := node.Cid()
		store.logger.Debugf("Block %s has unready ancestors", current)
		return ancestorResult.UnprocessedAncestors, nil
	}

	jobResult, err := processor.checkJobReadiness(ctx, root)
	if err != nil {
		return nil, err
	}
	...
	if err := processor.mergeJobDeltas(ctx, root, jobResult.JobNodes, getter); err != nil {
		return nil, err
	}

	if err := processor.updateHeads(ctx, root, rootPrio, ancestorResult.HeadsToReplace); err != nil {
		return nil, err
	}
